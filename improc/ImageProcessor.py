##########################################################
# this file contains a useful class for peakfinding and
# motion correction, both leveraging template filtering
# to smoothen an image. jit'd for performance when calling
# the corresponding functions a bunch but comment those
# out if you're just running the code once
...
# some other image reigstration packages, not used
...
class ImageProcessor:
    ...
    # function to calculate motion correction
    def calculate_moco(self, frame1, frame2):

        return template_moco(
            frame1,
            frame2,
            template_filter=self.template_filter,
            fb_threshold_margin=self.ops["fb_threshold_margin"],
            med_filt_size=self.ops["med_filt_size"],
        )
...
def template_moco(
    ...
    # first part is same as image_filtering but take some steps out
    # apply threshold
    threshold = np.median(frame1) + fb_threshold_margin
    frame1 = (frame1 > threshold) * frame1
    frame2 = (frame2 > threshold) * frame2
    ...
    # that doesn't matter for calculating offsets
    frame1 = cv2.matchTemplate(frame1, template_filter, cv2.TM_CCOEFF)
    frame2 = cv2.matchTemplate(frame2, template_filter, cv2.TM_CCOEFF)

    # calculate globmo
    xoff, yoff = cross_correlation_shifts(frame1, frame2)
